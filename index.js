...
"𝐁𝐲 𝐁𝐞𝐛𝐨",
"𝐕𝐞𝐫𝐭𝐞𝐱 𝐒𝐞𝐫𝐯𝐢𝐜𝐞",
"قمة الإبداع والتقنية لتقديم أفضل خدمة",
"𝐕𝐞𝐫𝐭𝐞𝐱 𝐒𝐞𝐫𝐯𝐢𝐜𝐞",
"𝐁𝐲 𝐁𝐞𝐛𝐨"
];
...
index = (index + 1) % statuses.length; // Loop through the statuses array
...
let optionss = [{
  ...
  value: 'buying2',
  emoji : `<:Vertex_Yes:1274492311425912874>`,
...
},       
{
  ...
  value: 'rest',
  emoji : `<:Vertex_rol2:1274492263027839007>`,
  selected: false
}
...
client.on('messageCreate', async (message) => {
  if (message.content.startsWith('sel')) {
if (!message.member.permissions.has(Permissions.FLAGS.ADMINISTRATOR))return;
    const ment = new MessageSelectMenu()
    .setCustomId('hope')
    .setPlaceholder('قم باختيار خدمة من القائمة')
    .setMaxValues(1)
    .setMinValues(1)
    .addOptions(optionss)
  ...
  const row = new MessageActionRow()
    ...
    message.channel.send({
     embeds: [new MessageEmbed().setDescription(`**لشراء احد خدماتنا يرجي اختيار نوع الخدمة وااستكمال باقي الخطوات مع البوت**
...
.setImage("https://cdn.discordapp.com/attachments/1274491304084246632/1274491486628614194/selfbot.gif?ex=66c27231&is=66c120b1&hm=f24327be368457daf105c0cd9135a1aa0baadbdaa92a60e861c6577eb5b747d3&")
.setThumbnail(message.guild.iconURL({dynamic : true}))
.setFooter({text : `By : Bebo | Vertex Service` , value : `${message.guild.name}`})
.setColor('003546')
.setThumbnail(message.guild.iconURL({dynamic : true}))
.setTitle('Vertex Service')],
      components: [row]
    });
  }
});
...
client.on("messageCreate", async (message) => {
  if (message.content.startsWith('delself')) {
    // هنا تقوم بتحديد معرف الكاتيجوري الثابت
    const categoryId = "1272836149349716072";

    // التأكد من صلاحيات المستخدم
    if (!message.member.permissions.has("ADMINISTRATOR")) {
      return message.reply(
        "**You do not have permissions to Use This Command **<a:1243473834997448724:1265410755457454100>",
      );
    }
    ...
    // حذف جميع الرومات داخل الكاتيجوري
    for (const channel of category.children.values()) {
      await channel.delete();
    }

    message.reply(
      "**Done Delete All Closed Tickets In Category** <:Vertex_Yes:1274492311425912874>",
    );
  }
});
...
`)]});
member.send({embeds: [new MessageEmbed().setTitle("Vertex Services").setDescription(`تم الاشتراك في خدمة : تلفيل كتابي`)]})
...
member.send({embeds : [new MessageEmbed().setDescription(`** لا تنسي تقييم خدماتنا من فضلك <#1272836192693653526>**`)]})
...
تم الشراء من قبل : ${member}
`)]});
member.send({embeds: [new MessageEmbed().setTitle("Vertex Services").setDescription(`تم الاشتراك في خدمة : تلفيل صوتي`)]})
              const bart = new MessageButton()
        .setLabel('قفل')
        .setCustomId('closeart')
        .setStyle('DANGER')
...
تم الشراء من قبل : ${member}
`)]});
member.send({embeds: [new MessageEmbed().setTitle("Vertex Services").setDescription(`تم الاشتراك في خدمة : اوتو رياكشن`)]});
      const bart = new MessageButton()
        .setLabel('قفل')
        .setCustomId('closeart')
        .setStyle('DANGER')
...
تم الشراء من قبل : ${member}
`)]});
member.send({embeds: [new MessageEmbed().setTitle("Vertex Services").setDescription(`تم الاشتراك في خدمة : حساب اونلاين`)]});
      const bart = new MessageButton()
        .setLabel('قفل')
        .setCustomId('closeart')
        .setStyle('DANGER')
